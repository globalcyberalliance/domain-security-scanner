...
func (s *Server) registerScanRoutes() {
	...
	huma.Register(s.router, huma.Operation{
		...
		if s.Advisor != nil {
			result.Advice = s.Scanner.CheckAll(result.ScanResult.Domain, result.ScanResult.BIMI, result.ScanResult.DKIM, result.ScanResult.DMARC, result.ScanResult.DNSSEC, result.ScanResult.MX, result.ScanResult.SPF, result.ScanResult.STS, result.ScanResult.STSPolicy)
		}
	...
	huma.Register(s.router, huma.Operation{
		...
		for _, result := range results {
			...
			if s.Advisor != nil && result.Error != scanner.ErrInvalidDomain {
				res.Advice = s.Scanner.CheckAll(result.Domain, result.BIMI, result.DKIM, result.DMARC, result.DNSSEC, result.MX, result.SPF, result.STS, result.STSPolicy)
			}
